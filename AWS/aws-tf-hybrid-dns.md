...
- Coded **all** infrastructure steps in *Terraform* including vpc peering, vpc peering inter region routes, route 53 inbound and outbound endpoints and forward rules, etc. instead of CloudFormation (`HybridDNS.yaml`) for initial/base infrastructure.
...
#### CFT stack missing config causing DNS outbound requests and Systems Manager to fail:
...
- VPC Endpoint of com.amazonaws.<region>.s3 Gateway type is required on both aws and onprem VPC's in order for Systems Manager to connect successfully to private only ec2 instances: 
