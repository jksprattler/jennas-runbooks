...
### Architecture
The us-east-2 region will be hosting the `micros4l-onprem` VPC with a prefix of 192.168.10.0/24 also containing 2 private subnets. Two t2.micro EC2 instances called `micros4l-onpremdnsa/b` configured with Bind and the zone `corp.microgreens4life.org` will be deployed here in a separate subnet/availability zone. Additionally, the zone for `aws.microgreens4life.org` will be configured on these servers with forwarders set to the AWS Inbound endpoint IP addresses in order to forward requests for the Web subdomain to the AWS Route 53 resolvers. A third basic t2.micro EC2 instance called `micros4l-onpremapp` will be deployed which we'll use for testing DNS resolution from "on-prem" to AWS (us-east-1).
